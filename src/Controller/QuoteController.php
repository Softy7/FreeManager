...
final class QuoteController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, HttpClientInterface $client): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $entityManager->flush();
            $this->sentToPowerAutomate($quote, $client);
    ...
    */
    ...
    public function sentToPowerAutomate(Quote $quote, HttpClientInterface $client) {
    
    $url = '';

    $client->request('POST', $url, [
        'json' => [
            'quote_description' => $quote->getReference(),
            'client_email' => $quote->getClient()->getEmail(),
            'client_name'  => $quote->getClient()->getName(),
            'amount'       => $quote->getTotalHt(),
            'id'  => $quote->getId(),
        ],
    ]);
